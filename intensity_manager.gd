...
## Tracks combo, intensity, and tier state.
## Drives GameEvents.intensity_changed and GameEvents.tier_changed signals.
...
func _update_tier() -> void:
	...
	if new_tier != current_tier:
		...
		GameEvents.tier_changed.emit(new_tier, old_tier)
...
func reset_combo() -> void:
	...
	if current_tier != Tier.CALM:
		...
		GameEvents.tier_changed.emit(Tier.CALM, old_tier)
